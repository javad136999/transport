...
export default function HomePage() {
  ...
  return (
    <main className="min-h-[100dvh] bg-[#effcff] text-slate-900">
        ...
        <div className="mx-auto flex max-w-7xl flex-col px-4 py-5 md:px-8 md:py-7">
          <header className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-black tracking-tight text-slate-900 md:text-lg">سامانه مدیریت حمل فاضلاب</div>
                <div className="mt-1 text-[10px] font-semibold text-cyan-800 md:text-xs">رصد هوشمند حمل فاضلاب بهداشتی عسلویه</div>
              </div>
            </div>
            <Link href="/login" className="rounded-xl border border-cyan-300/80 bg-white/90 px-4 py-2.5 text-xs font-bold text-slate-900 shadow-[0_8px_25px_rgba(0,150,200,.12)] backdrop-blur-md transition hover:border-cyan-500 hover:bg-white">
              ورود به سامانه
            </Link>
          </header>

          <div className="mt-24 max-w-3xl md:mt-28">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-300 bg-white/80 px-3 py-1.5 text-xs font-bold text-slate-900 shadow-sm backdrop-blur-md">
              <span className="h-2 w-2 animate-pulse rounded-full bg-cyan-500 shadow-[0_0_12px_#00BFFF]" />
              سامانه رصد و کنترل عملیات
            </div>
            <h1 className="text-4xl font-black leading-[1.25] tracking-tight text-slate-950 drop-shadow-sm md:text-6xl">
              حمل فاضلاب،<br />
              <span className="text-cyan-700">قابل رصد و قابل اعتماد</span>
            </h1>
            <p className="mt-6 max-w-2xl text-sm font-medium leading-8 text-slate-800 md:text-base">
            ...
            <div className="mt-7 flex flex-wrap gap-3 text-xs">
              <div className="flex items-center gap-2 rounded-xl border border-cyan-200 bg-white/85 px-4 py-3 font-semibold text-slate-800 shadow-sm backdrop-blur-md"><Wifi size={15} className="text-cyan-700" /> رصد عملیاتی</div>
              <div className="flex items-center gap-2 rounded-xl border border-cyan-200 bg-white/85 px-4 py-3 font-semibold text-slate-800 shadow-sm backdrop-blur-md"><ShieldCheck size={15} className="text-cyan-700" /> کنترل مجوزها</div>
              <div className="flex items-center gap-2 rounded-xl border border-cyan-200 bg-white/85 px-4 py-3 font-semibold text-slate-800 shadow-sm backdrop-blur-md"><Activity size={15} className="text-cyan-700" /> رهگیری مأموریت</div>
            </div>
          </div>
        </div>
      ...
      <section className="relative z-10 mx-auto -mt-12 max-w-5xl px-4 md:-mt-16 md:px-8">
        <div className="overflow-hidden rounded-3xl border border-cyan-200 bg-white/95 p-5 shadow-[0_20px_80px_rgba(0,150,190,.16),0_0_45px_rgba(0,191,255,.10)] backdrop-blur-2xl md:p-7">
          <div className="mb-5 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="grid h-11 w-11 place-items-center rounded-2xl bg-cyan-50 text-cyan-700"><Search size={20} /></div>
              <div>
                <h2 className="font-extrabold text-slate-900 md:text-lg">استعلام پلاک تانکر</h2>
                <p className="mt-1 text-[11px] font-medium text-slate-600">بررسی ثبت تانکر و وضعیت مأموریت جاری</p>
              </div>
            </div>
            <div className="hidden items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-[10px] font-bold text-cyan-800 sm:flex">
          ...
          <form onSubmit={handleInquiry} className="flex flex-col gap-3 sm:flex-row">
            <input
              className="min-w-0 flex-1 rounded-2xl border border-cyan-200 bg-cyan-50/60 px-4 py-4 text-center text-lg font-bold tracking-[.25em] text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-300/30"
              dir="ltr"
          ...
          {result && (
            <div className={`mt-5 rounded-2xl border p-4 ${result.status === "unauthorized" || result.status === "not_registered" ? "border-red-200 bg-red-50" : "border-cyan-100 bg-cyan-50/60"}`}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                {result.vehicle && <span className="font-mono text-sm font-bold text-slate-800">{result.vehicle.plate_normalized}</span>}
              </div>
              ...
              {result.mission && (
                <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 text-xs">
                  <AlertTriangle size={15} className="text-status-alert" />
                  <span className="font-semibold text-red-800">مأموریت فعال:</span>
                  <strong className="text-slate-900">{result.mission.mission_no ?? "ثبت‌شده"}</strong>
                  <span className="font-bold text-red-700">{String(result.mission.status ?? "").replaceAll("_", " ")}</span>
                </div>
              )}
            </div>
          )}
...
function Info({ label, value, critical = false }: { label: string; value: string; critical?: boolean }) {
  return <div className={`rounded-xl border p-3 shadow-sm ${critical ? "border-red-200 bg-red-50" : "border-cyan-100 bg-white"}`}><div className="text-[10px] font-medium text-slate-600">{label}</div><div className={`mt-1 font-bold ${critical ? "text-red-700" : "text-slate-900"}`}>{value}</div></div>;
}

function Feature({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return <div className="group rounded-2xl border border-cyan-100 bg-white p-5 shadow-[0_12px_35px_rgba(0,150,190,.08)] transition hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-[0_15px_40px_rgba(0,150,190,.14)]"><div className="mb-3 flex items-center gap-2 text-cyan-700">{icon}<span className="font-bold text-slate-900">{title}</span></div><p className="text-xs font-medium leading-7 text-slate-600">{text}</p></div>;
}
